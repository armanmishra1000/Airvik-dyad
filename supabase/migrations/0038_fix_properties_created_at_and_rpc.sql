-- Fix properties table missing created_at column and update RPC to use order by id
-- This resolves error 42703: column "created_at" does not exist
begin;

-- Add created_at column to properties table for consistency with other tables
alter table public.properties
  add column if not exists created_at timestamptz not null default now();

-- Recreate get_monthly_availability to use order by id instead of created_at
-- This makes the function resilient even if created_at is missing
...
comment on function public.get_monthly_availability(date, uuid[])
  is 'Aggregates per-room-type availability, respecting seasonal and property closures. Fixed to use order by id instead of created_at.';
