-- Fix availability calendar to show ALL room types, even those without rooms
-- Changes INNER JOIN to LEFT JOIN so room types with 0 rooms are included
-- This resolves the issue where newly created room types don't appear in the calendar

begin;

-- Recreate get_monthly_availability with LEFT JOIN instead of INNER JOIN
...
comment on function public.get_monthly_availability(date, uuid[])
  is 'Aggregates per-room-type availability with LEFT JOIN to include room types without rooms. Shows all configured room types regardless of whether they have units assigned.';
