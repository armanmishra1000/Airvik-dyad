-- Consolidate create_reservations_with_total into a single, non-ambiguous signature
...
begin
  ...
  if coalesce(btrim(p_booking_id), '') = '' then
    ...
    -- Normalize provided booking IDs to A#### format
    if upper(p_booking_id) ~ '^A[0-9]+$' then
      v_booking_id := upper(p_booking_id);
    else
      -- Strip all non-digits and prepend A when digits exist
