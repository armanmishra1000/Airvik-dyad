...
begin
  for ddl in
    select format(
      'drop function if exists public.create_reservations_with_total(%s);',
      pg_get_function_identity_arguments(p.oid)
    )
    from pg_proc p
    join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public'
      and p.proname = 'create_reservations_with_total'
  loop
    execute ddl;
  end loop;
end;
...
begin
  ...
  if coalesce(btrim(p_booking_id), '') = '' then
    ...
    if upper(p_booking_id) ~ '^A[0-9]+$' then
      v_booking_id := upper(p_booking_id);
    else
      v_booking_id := regexp_replace(p_booking_id, '\D', '', 'g');
      if coalesce(v_booking_id, '') = '' then
        raise exception 'Invalid booking_id % (should resemble A6504 or include digits)', p_booking_id
          using errcode = '22023';
      end if;
      v_booking_id := concat('A', v_booking_id);
    end if;
  end if;
...
begin
  ...
  if v_count <> 1 then
    raise exception 'Expected exactly one create_reservations_with_total function, found %', v_count;
  end if;
end;
$$ language plpgsql;
