  ...
  -- ✅ Drop the CURRENT production overload you pasted (no p_custom_totals, different arg order)
  ...
  -- Existing legacy overloads (keep these)
  drop function if exists public.create_reservations_with_total(
    text,
    uuid,
    uuid[],
  ...
  -- Recreate the canonical function definition so the schema cache picks up the correct
  -- signature that includes p_custom_totals.
